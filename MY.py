...
class MyCache:
    ...
    def access(self, frame_number):
        ...
        while self.layer2.node_cnt > 204800:
            ...
            self.trash.insert(0,NotNeeded_node.frame)
    ...
    def search(self, frame_number):
        ...
        """
        found_node = self.trash.fnd(Frame(frame_number))
        if found_node:
            return 3, found_node.frame        
        """
        # 在垃圾桶裡面找到
        for f in self.trash:
            if f.frame_number == frame_number:
...
with open("reuse_distance.bin", 'wb')  as  output_file_2 :
    pickle.dump(cache.reuse_distance, output_file_2)