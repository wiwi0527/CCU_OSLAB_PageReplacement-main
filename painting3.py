...
plt.scatter(range(len(deviations)), deviations, color='blue', label='Reuse Distance', s=0.1)

with open("reuse_distance.bin", 'rb') as reuse_distance_file:
    reuse_distances = pickle.load(reuse_distance_file)


plt.scatter(range(len(reuse_distances)), reuse_distances, color='red', label='Reuse Distance', s=0.1)
